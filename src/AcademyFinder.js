...
// API 선택 (GitHub Pages 또는 로컬 서버)
const USE_GITHUB_DATA = true; // GitHub Pages 데이터 사용

// GitHub Pages API 또는 로컬 API 사용
import { fetchAcademies as githubFetchAcademies, fetchRegions as githubFetchRegions, fetchCategories as githubFetchCategories } from './GitHubDataAPI';

// API 기본 URL (로컬 서버용)
const API_BASE_URL = process.env.NODE_ENV === 'production' 
  ? 'https://loac-9ec81.web.app/api'  // Firebase Functions URL (향후 사용)
  : 'http://localhost:3001/api';      // 로컬 개발 서버

// 로컬 서버 API 함수들
const localFetchAcademies = async (params = {}) => {
  try {
    console.log('API에서 학원 데이터 로드 중...', params);
    
    const queryParams = new URLSearchParams();
    if (params.search) queryParams.append('search', params.search);
    if (params.region && params.region !== '전체') queryParams.append('region', params.region);
    if (params.category && params.category !== '전체') queryParams.append('category', params.category);
    if (params.page) queryParams.append('page', params.page);
    if (params.limit) queryParams.append('limit', params.limit);

    const response = await fetch(`${API_BASE_URL}/academies?${queryParams}`);
    ...
    console.log('API 응답 받음, 학원 수:', result.data?.length || 0);
...
// 사용할 API 함수 선택
const fetchAcademies = USE_GITHUB_DATA ? githubFetchAcademies : localFetchAcademies;
const fetchRegions = USE_GITHUB_DATA ? githubFetchRegions : localFetchRegions;
const fetchCategories = USE_GITHUB_DATA ? githubFetchCategories : localFetchCategories;
...
const AcademyFinder = () => {
        ...
        <div className="mt-2 text-sm text-gray-600">
          {loading ? '로딩 중...' : (
            <div className="flex justify-between items-center">
              <span>총 {pagination.total}개 학원</span>
              {searchQuery && (
  ...
  const MapView = () => {
    ...
    }, [academies, selectedAcademy]); // eslint-disable-line react-hooks/exhaustive-deps
          ...
          <div className="absolute top-4 left-4 right-4 bg-white rounded-lg shadow-lg p-4 z-10">
            <div className="flex justify-between items-start">
              <div className="flex-1">
                <h4 className="font-bold text-lg mb-1">{selectedAcademy.FACLT_NM}</h4>
                <p className="text-sm text-gray-600 mb-2">
                  {selectedAcademy.REFINE_ROADNM_ADDR || selectedAcademy.REFINE_LOTNO_ADDR}
                </p>
                <div className="flex items-center space-x-4 text-sm">
                  ...
                  {selectedAcademy.TELNO && (
                    <div className="text-blue-600 font-medium">{selectedAcademy.TELNO}</div>
                  )}
                </div>
                <div className="flex flex-wrap gap-1 mt-2">
                  <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">
                    {selectedAcademy.CRSE_CLASS_NM}
                  </span>
                </div>
              </div>
              <button 
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="bg-white border-b p-4">
        <div className="flex gap-3">
          <div className="flex-1">
            <input
              type="text"
              placeholder="학원명, 지역, 과목, 원장명, 주소, 전화번호로 검색해보세요..."
              value={searchQuery}
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* 학원 리스트 */}
        <div className="w-full lg:w-1/2 bg-white border-r min-h-96">
          <AcademyList />
        </div>
